...
function RewardPointsCard({ entries, travelEntries, dbMembers, onSaveError }) {
  ...
  return (
    <Card className="px-5 py-4 mb-4">
      <View className="flex-col sm:flex-row sm:items-center sm:justify-between" style={{ gap: 16 }}>
      ...
      {settling && (
        <View className="mt-4 pt-4 border-t border-ink/10">
          <Text className="font-body text-sm text-ink mb-3">
            ...
            or pay it down; this just logs points that actually changed hands.
          </Text>
            ...
            <Text className="font-body text-xs text-muted-text mt-3 mb-3">
              {previewBalance.status === 'settled' ? (
                ...
                <>
                  ...
                  <Text className="font-mono-bold text-ink">{Math.round(previewBalance.amount).toLocaleString('en-IN')} pts</Text>.
                </>
              )}
            </Text>
          )}
