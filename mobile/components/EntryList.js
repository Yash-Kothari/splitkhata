...
// commits - matches web's EntryList.jsx exactly.
...
export default function EntryList({
  ...
  // its undo window. The entry stays hidden from the list the instant
  // delete is tapped (optimistic), but nothing actually leaves Firestore
  // until the timer in handleDelete below fires, unless handleUndo cancels
  // it first.
  ...
  // first - the delete is real the moment it's tapped, this is a grace
  // period, not a second confirmation dialog to stick around for.
  ...
  return (
    <>
        ...
        <View className="rounded-t-2xl border border-b-0 border-ink/10 bg-paper-card px-4 pt-4 pb-3">
          ...
          {!loading && (entries || []).length > 0 && (
            <TextInput
              ...
              className="font-body text-sm text-ink border border-ink/15 rounded-xl px-3 py-2 mt-2.5 bg-paper shadow-2xs"
            />
          )}
        </View>
      ...
      {pendingDeleteList.length > 0 && (
        // Not position:absolute - EntryList is rendered inside each screen's
        // own ScrollView (unlike cards.js's toast, which floats over a
        // sibling of its ScrollView), so an absolutely-positioned toast here
        // would be scoped to the scrollable content instead of the
        // viewport. A normal in-flow banner right after the list is
        // reliably visible either way.
        <View className="mx-4 mb-4" style={{ gap: 8 }}>
          {pendingDeleteList.map(({ entry }) => (
            <View key={entry.id} className="flex-row items-center justify-between gap-3 rounded-xl bg-ink px-4 py-3">
              ...
              <Pressable onPress={() => handleUndo(entry.id)} hitSlop={8}>
